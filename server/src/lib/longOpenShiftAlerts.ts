...
//
// Historical note: while midnight rollover existed (closing-and-reopening
// an equivalent entry at every local midnight — see the old
// midnightRollover.ts, since replaced by midnightCutoff.ts's "close, never
// continue" design), the current open entry's own started_at could reset at
// every local midnight it crossed, so a shift genuinely open 20 hours could
// show a `started_at` only an hour or two old. walkShiftStart's chain walk
// below exists for that reason and still correctly resolves any such
// historical chain in production data; going forward, midnight cutoff never
// lets an open entry's started_at go stale in the first place (an entry
// currently open can only ever have started earlier today), so the walk is
// now typically a single-hop no-op for any newly-created entry — kept as-is
// since it costs nothing extra and remains correct either way.
...
// transition AND (historically) midnight rollover guaranteed for its own
// boundary — with NO activity/row/carrier/entry_type restriction (unlike
// mobileTime.ts's accumulateChainSeconds, which is deliberately scoped to
// "still on this specific job"; this is answering "still on the clock at
// all"). Stops at the first real gap (no entry ends exactly there), which is
// the employee's last genuine idle point. Exported for longShiftAdminEnd.ts
// — the End Work admin action needs the exact same "true continuous streak"
// boundary this alert is computed against, both to show it in the
// confirmation modal and to reject a proposed end time that's before the
// shift genuinely started.
