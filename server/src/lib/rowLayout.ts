// Deterministic greenhouse row placement math — the authoritative,
// server-side twin of web/src/lib/rowLayout.ts (kept in sync by hand; this
// app has no shared package between server/ and web/, same reasoning as
// lib/timezone.ts). The server never trusts client-submitted row x/y — it
// recomputes every row's geometry from the batch parameters using these
// exact functions before writing anything, so a buggy or malicious client
// can never persist geometry the server itself didn't derive.
//
// ---------------------------------------------------------------------------
...
// PhasePositionEditor's EPSILON on the client.
const EPSILON = 0.001;
...
// batch — see the route for why this can't be inferred from geometry
// alone: a row's orientation alone doesn't distinguish "started from
